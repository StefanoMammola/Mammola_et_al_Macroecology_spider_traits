...
output_dir <- "Models/" #Folder where to save the data
...
alpha.null <- readRDS("F_metrics/SES_FR.rds")
...
table_disp<- alpha_ses %>% mutate(d = value > 0) %>% pull("d") %>%  tabyl %>% mutate(percent = percent*100) 
...
table_gls <- mod3 %>%  
  ...
  gt_theme_538() %>% 
  gt_merge_stack(col1 = term, col2 = unit, colors = c("gray40","gray60"))
  
table_gls %>%  gtsave("table_gls.html")
